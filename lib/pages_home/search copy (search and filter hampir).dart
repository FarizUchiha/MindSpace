...
class _SearchPageState extends State<SearchPage> {
  ...
  void initState() {
    ...
    _searchController.addListener(() {
      if (_searchController.text.isEmpty && selectedCategories.isEmpty) {
        setState(() {
          filteredCourses.clear();
        });
      } else {
        _fetchCoursesWithFilters(); // Fetch courses based on filters and search
      }
    });
  }
  ...
  Future<void> _fetchCoursesWithFilters() async {
    setState(() {
      _isLoading = true;
    });
    try {
      ...
      if (selectedCategories.isNotEmpty) {
        final categoryIdsResponse = await Supabase.instance.client
            .from('category')
            .select('id, name')
            .filter('name', 'in', '(${selectedCategories.join(",")})');
      ...
      if (_minPriceController.text.isNotEmpty) {
        query = query.gte('price', int.tryParse(_minPriceController.text) as Object);
      }
      if (_maxPriceController.text.isNotEmpty) {
        query = query.lte('price', int.tryParse(_maxPriceController.text) as Object);
      }
  ...
  Future<void> _refreshCourses() async {
    await _fetchCoursesWithFilters();
  }
